...
public class Health : MonoBehaviour {
	...
	void Start() {
		if(OnSetHealth != null) OnSetHealth(currentHealth);
	}
	...
	public float ChangeHealth(HealthEvent amount) {
		if(!disableChanges) {
			float healthChange = currentHealth;

			currentHealth = Mathf.Clamp(currentHealth + amount.amount, 0, maxHealth);
			if(OnSetHealth != null) OnSetHealth(currentHealth);

			healthChange = currentHealth - healthChange;
